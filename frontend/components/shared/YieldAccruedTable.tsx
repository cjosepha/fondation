...
const YieldAccruedTable = () => {
    ...
    const eventAbi = parseAbiItem(
        "event YieldAccrued(uint256 amount, uint256 when)"
    );
    ...
    const addEventsFromLogs = (logs: Log[]) => {
        ...
        const yieldAccruedEvents = events.concat(eventsFromLogs);
    ...
    return (
        <Table>
            <TableCaption >wBTC accrued</TableCaption>
            <TableHeader>
                <TableRow>
                    <TableHead className="w-[100px]">Amount (wBTC)</TableHead>
                    <TableHead className="text-right">Accrued On</TableHead>
                </TableRow>
            </TableHeader>
            <TableBody>
                {events.map((event) => (
...
export default YieldAccruedTable;